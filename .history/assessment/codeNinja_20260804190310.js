/* 
Create a JavaScript object called shoppingCart that represents a customer's shopping cart.

The object should have a cartItems property, which is an array containing the 
items in the cart.
 Each item should be represented as an object with a name, price, and quantity.

Your shoppingCart object should have the following methods:

1. addItem(name, price, quantity)
Adds a new item to the cart.
Example: shoppingCart.addItem("Keyboard", 25000, 2);

2. calculateTotal()
Calculates and returns the total cost of all items in the cart.

3. displayCart()
Displays each item in the cart along with its name, price, and quantity.

4. checkout()
Checks if the cart is empty.
If the cart is empty, display "Your cart is empty".
Otherwise, display the total amount to be paid and clear the cart.

5. getCartItems()
Returns the current items in the cart.

6. clearCart()
Removes all items from the cart.
It sets cartItems back to an empty array.

*/





const shoppingCart={

    addItem:{
        phone:{
        name: Charger,
        price: 4000,
        quantity:2,
        },

        charger:{name: Chargers,
        price: 4000,
        quantity:2,
        }
       
    },
 

calculatetotal: function calculatetotal (){

     this.addItem.phone.price +  this.addItem.charger.price 
},

displaycart:function displaycart(){

     this.addItem.phone.name + this.addItem.phone.name
},

checkout: function checkout(){

    if(this.addItem < 0){
        console.log("cart is empty")
    }
    else{
        console.log(this.addItem)
    }
},

getCartItems: function getCartItems(){
    this.addItem.name
},

clearCart: function clearCart(){
    this.addItem.name = ""
}

}


console.log(shoppingCart.addItem.name);
shoppingCart.calculatetotal();
shoppingCart.checkout();
shoppingCart.clearCart();
shoppingCart.displaycart();
shoppingCart.getCartItems();