//   const yourName = prompt("What is your name?");
//     const yourAge = Number( prompt("How old are you?"));
//     const hobby = prompt("What is your favorite hobby");
//     const currentYear = 2026;
//     const birthYear =  currentYear -  yourAge
//     console.log(`Welcome! ${yourName}, You were born in the year ${birthYear}, Your favorite hobby is ${hobby} `);

//     let age = 20;

//     const name = "peter";

// console.log(name)

// + is used on math   when + is used for joining string

//  let add = 2+2;

// let num  = Math.random() * 10;

// console.log(num);

// const name = "peter";

// console.log("my name is :"  + name )

// const age = 40;

// const name = "favour";
// const name1 = "Barista Ada";
//  const output = "Hello, " + name + "! You are " + age + "years old." ;

// console.log(output);

// const output2 =`Hello, ${name1}! You are ${age} years old.`
// console.log(output2);

// let A =5;
// let B = 5;

// const add = A + B;

// // let name;

// console.log(add)

// const firstName = prompt("What is your first name");
// const favoriteNumber = prompt(" what's your favorite number");
// favoriteNumber = Number(favoriteNumber)
// if (isNaN (favoriteNumber)) {
//   alert("Please enter a valid number")
//   return;
// }
// firstName = firstName.toLowerCase();
// const userName = firstName + favoriteNumber;
// alert("your user name is:" + userName);

// function generateUsername() {
//     const firstName = prompt("What is your first name?");
//     let favoriteNumber = prompt("What's your favorite number?");

//     favoriteNumber = Number(favoriteNumber);

//     if (isNaN(favoriteNumber)) {
//         alert("Please enter a valid number.");
//         return;
//     }

//     const userName = firstName.toLowerCase() + favoriteNumber;

//     alert("Your username is: " + userName);
// }

// generateUsername();

// let a = 10;
// let b = "5";

// console.log(a + b);
// console.log(a - b);


  //  let price = 20;
  //  let quantity = 3;
  //  console.log(`total cost: $$${price + quantity}`);


    // let x = 4;
    // let y = 2;
    // console.log(" Result: " + x + y);
    // console.log(" Result: " + (x + y));

// let firstName = "Tunde";
// let lastName = " okafor";
// let fullName = firstName + "" + lastName;

// console.log("welcome," + fullname + "!");


// let itemPrice = "15";
// let total  = itemPrice + 5;
//     console.log(`your total is $${total}`);

// let item = [1,3,5];
// // let price = [];
// // let quantity =[]

// item.push[6];

// //  function additems(item,price, quantity){
// //      const total = `i bought ${quantity} piece of ${item} at $${price} `
// //        console.log(total)
// //  }


// //  additems(value,4000,3);


// let item = ["yam"];
// let price = [];
// let quantity =[]

//  function additems(item,price1, quantity){
//      const total = `i bought ${quantity} piece of ${item} at $${price1} `
//        console.log(total)
//  }

//  additems(item, 4000,3);


 let a = 2;
 let b = 4;
 let c = 5;

function addnumber(num1,num2,num3){
let result =m1 - num3 * num2 nu
    console.log(`add ${num1} - ${num3}  * ${num2} `)
}
addnumber(2,4,5);